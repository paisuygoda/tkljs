      ...
      var members = $gameParty.members();
		...
		members.forEach(function(actor) {
			...
			while (actor._jobLevel[actorClass.id] > jobLevel) {
				var code = actor._actorId * 100 + actorClass.abilities[jobLevel]['base_code'];
...
BattleManager.displayAbilities = function() {
    ...
    if (abilities.length > 0) {
        ...
        abilities.forEach(function(ability) {
            $gameMessage.add('\\>' + Yanfly.Param.AbilityTextFormat1.format(ability.actorName));
            $gameMessage.add('\\>' + Yanfly.Param.AbilityTextFormat2.format(ability.abilityName));
        });
    }
};
...
BattleManager.displayJpGain = function() {
    ...
    //$gameParty.members().forEach(function(actor) {
		...
		var text = fmt.format(Yanfly.Param.JpThisBattle, Yanfly.Param.Jp);
...
Game_Troop.prototype.jpTotal = function() {
    return Yanfly.Param.JpThisBattle;
};
